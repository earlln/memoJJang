...
> **메모짱 ver1.2 by Earlln.com** — 심플하면서도 세련된 윈도우 메모장
...
![버전](https://img.shields.io/badge/version-1.2.0-2F6FED)
...
창 제목은 항상 `<파일 이름> - 메모짱 ver1.2 by Earlln.com` 형식이며,
...
- **탭 편집** — 한 창에서 여러 파일. 탭마다 실행 취소 기록과 캐럿 위치가 따로 유지됩니다.
  탭을 오른쪽 클릭하면 새 탭 / 탭 닫기 / 다른 탭 모두 닫기 / 오른쪽·왼쪽 탭 모두 닫기를 쓸 수 있습니다.
- **Markdown 미리 보기** (`Ctrl+Shift+M`) — 편집기 옆에 나란히 열립니다. `.md` 파일은 자동으로 열립니다.
  앱 테마를 그대로 따르고, 원격 이미지는 내려받지 않습니다.
- **열 단위(사각형) 편집** — `Alt` + 드래그 또는 `Alt+Shift+C`.
  여러 줄의 같은 열에 한 번에 입력·삭제하고, 사각형 단위로 복사·붙여넣기 할 수 있습니다.
...
| `Ctrl+Z` / `Ctrl+Y` | 실행 취소 / 다시 실행 | | `Esc` | 찾기 막대 · 열 선택 닫기 |
| `Ctrl+Shift+M` | Markdown 미리 보기 | | `Alt`+드래그 / `Alt+Shift+C` | 열 단위 선택 |
